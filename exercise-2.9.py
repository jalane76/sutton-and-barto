...
def main():
    ...
    doUCB = True
    doOptimistic = True
    doConstantEpsilon = True

    x_vals = [1.0 / 128.0, 1.0 / 64.0, 1.0 / 32.0, 1.0 / 16.0, 1.0 / 8.0, 1.0 / 4.0, 1.0 / 2.0, 1.0, 2.0, 4.0]
    ax = plt.subplot(111)
    ax.set_title('Parameter study, nonstationary bandit', loc='left')
    ...
    if doEpsilon:
        ...
        for epsilon in epsilons:
            ...
            for run in range(max_runs):
                ...
                bandit = bandido.Bandit(k=k, step_sigma=stddev)
                ...
                for step in range(max_steps):
                    ...
                    if step > max_steps / 2:
                        average_steps_reward = average_steps_reward + (reward - average_steps_reward) / ((step + 1) - max_steps / 2)
    ...
    if doConstantEpsilon:
        print('Epsilon-greedy agent with constant step...')
        average_rewards = [math.nan for x in x_vals if x < epsilons[0]]
        for epsilon in epsilons:
            average_run_reward = 0
            print('epsilon = ', epsilon)
            for run in range(max_runs):
                ...
                bandit = bandido.Bandit(k=k, step_sigma=stddev)
                agent = bandido.Agent(k=k, epsilon=epsilon, alpha=0.1)
                average_steps_reward = 0
            ...
            print()  # basically a newline
        average_rewards = average_rewards + [math.nan for x in x_vals if x > epsilons[-1]]
        plt.plot(x_vals, average_rewards, label='Epsilon w/ const step')
